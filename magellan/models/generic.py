...
from magellan.models import BaseContentSubscription


class GenericWebsiteSubscription(BaseContentSubscription):
    """
    A generic website spider for sites without a more specific content
    subscription model.
    """
    base_url = models.CharField(max_length=255,
                                help_text="Full URL to page to begin spidering")
    domain = models.CharField(max_length=255,
                              help_text="Substring (of domain or otherwise) to limit links followed",
                              blank=True)
    depth = models.IntegerField(default=0,
                                help_text="How many pages deep to follow links; 0 for infinite")
    timeout = models.IntegerField(default=30, 
                                  help_text="Maximum time, per page, to wait for a response")
    ...
    def __unicode__(self):
        if self.user:
            return u"%s's subscription of %s, starting at %s" % (
                self.user, self.name, self.base_url
            )
        else:
            return u"%s - starting at: %s" % (self.name, self.base_url)


class GenericWebsiteSubscriptionForm(forms.ModelForm):
    class Meta:
        model = GenericWebsiteSubscription
        widgets = {
                   'login_details': forms.TextInput(attrs={'size': '3'})
                   }


class GenericWebsiteSubscriptionAdmin(admin.ModelAdmin):
    form = GenericWebsiteSubscriptionForm


admin.site.register(GenericWebsiteSubscription,
                    GenericWebsiteSubscriptionAdmin)
